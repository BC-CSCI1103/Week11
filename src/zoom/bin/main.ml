...
let initialModel file =
  let image = Image.readImage Sys.argv.(1) in
  let bmp = Image.toBitmap image in
  let width = float (cols bmp) in
  let height = float (rows bmp)
  in
  { image; width; height }
...
(* animation start *)
let go () =
  let model = initialModel Sys.argv.(1)
  in
  Animate.start
    model
    ~name: "Zoomer"
